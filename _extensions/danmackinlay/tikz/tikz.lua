...
local function compile_tikz_to_svg(code, user_opts, conf, basename)  -- Added conf and basename parameters
  ...
  -- Check for inkscape in PATH or at known macOS location
  local inkscape_cmd = 'inkscape'
  if not check_dependency('inkscape') then
    if os.execute('test -x /Applications/Inkscape.app/Contents/MacOS/inkscape') then
...
\end{document}
      ...
      local success_inkscape, inkscape_result = pcall(pandoc.pipe, inkscape_cmd, args, '')
